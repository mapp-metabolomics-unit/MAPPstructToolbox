---
title: "Data analysis of metabolomics and other omics datasets using the MAPPstructToolbox"
author: 
- name: Gavin R Lloyd
...
output:
  BiocStyle::html_document:
    ...
    toc_float: true
package: MAPPstructToolbox
abstract: |
    Data (pre-)processing and data analysis of Metabolomics and other omics datasets using struct and MAPPstructToolbox, including univariate/multivariate statistics and machine learning approaches.
vignette: >
  %\VignetteIndexEntry{Data analysis of metabolomics and other omics datasets using the MAPPstructToolbox}
...
# Introduction
The 'MAPPstructToolbox' includes an extensive set of data (pre-)processing and analysis tools for metabolomics and other omics, with a strong emphasis on statistics and machine learning. The methods and tools have been implemented using class-based templates available via the `struct` (Statistics in R Using Class-based Templates) package. The aim of this vignette is to introduce the reader to basic and more advanced MAPPstructToolbox-based operations and implementations, such as the use of `struct` objects, getting/setting methods/parameters, and building workflows for the analysis of mass spectrometry (MS) and nuclear magnetic resonance (NMR)-based Metabolomics and proteomics datasets. The workflows demonstrated here include a wide range of methods and tools including pre-processing such as filtering, normalisation and scaling, followed by univariate and/or multivariate statistics, and machine learning approaches. 

# Getting started
The latest version of `MAPPstructToolbox` compatible with your current R version can be installed using `BiocManager`.
...
# install MAPPstructToolbox and dependencies
BiocManager::install("MAPPstructToolbox")
```
...
## Introduction
PCA (Principal Component Analysis) and PLS (Partial Least Squares) are commonly applied methods for exploring and analysing multivariate datasets. Here we use these two statistical methods to demonstrate the different types of `struct` (STatistics in R Using Class Templates) objects that are available as part of the `MAPPstructToolbox` and how these objects (i.e. class templates) can be used to conduct unsupervised and supervised multivariate statistical analysis.
...
Both datasets are available as part of the MAPPstructToolbox package and already prepared as a `DatasetExperiment` object.
...
In `MAPPstructToolbox` mean centring and PCA are both model objects, and joining them using "+" creates a model_sequence object. In a model_sequence the outputs of the first object (mean centring) are automatically passed to the inputs of the second object (PCA), which allows you chain together modelling steps in order to build a workflow.
...
### Ontology
Within the `struct` framework (and `MAPPstructToolbox`) an `ontology` slot is provided to allow for standardardised definitions for objects and its inputs and outputs using the Ontology Lookup Service (OLS).
...
## Introduction
This vignette provides an overview of a `MAPPstructToolbox` workflow implemented to process (e.g. filter features, signal drift and batch correction, normalise and missing value imputation) mass spectrometry data. The workflow exists of methods that are part of the Peak Matrix Processing (`r Biocpkg('pmp')`) package, including a range of additional filters that are described in Kirwan et al., [2013](https://link.springer.com/article/10.1007/s00216-013-6856-7), [2014](https://doi.org/10.1038/sdata.2014.12).

Some packages are required for this vignette in addition `MAPPstructToolbox`:
...
The `MTBLS79_DatasetExperiment` object included in the `MAPPstructToolbox` package is a processed version of the MTBLS79 dataset available in peak matrix processing (`r Biocpkg('pmp')`) package. This vignette describes step by step how the `MAPPstructToolbox` version was created from the `pmp` version (i.e. 'Dataset 7:SFPM' from the Scientific Data publication - https://doi.org/10.1038/sdata.2014.12).

The `SummarizedExperiment` object from the `pmp` package needs to be converted to a `DatasetExperiment` object for use with `MAPPstructToolbox`.
...
wrapped into a `MAPPstructToolbox` object called `sb_corr`.
...
## Introduction
The aim of this vignette is to demonstrate how to 1) apply and validate Partial Least Squares (PLS) analysis using the MAPPstructToolbox, 2) reproduce statistical analysis in [Thevenot et al. (2015)](https://pubs.acs.org/doi/10.1021/acs.jproteome.5b00354) and 3. compare different implementations of PLS.
...
The third plot coloured by gender (C) is identical to Figure 2 of the `r Biocpkg("ropls")` package vignette. The `MAPPstructToolbox` package provides a range of PCA-related diagnostic plots, including D-statistic, scree, and loadings plots. These plots can be used to further explore the variance of the data.
...
The `ropls` package uses its own implementation of the (O)PLS algorithms. `MAPPstructToolbox` uses the `pls`
...
The plot is similar to fig.3 of the `r Biocpkg("ropls")` vignette. Differences are due to inverted LV axes, a common occurrence with the NIPALS algorithm (used by both `MAPPstructToolbox` and `ropls`) which depends on how the algorithm is initialised.

To compare the R2 values for the model in MAPPstructToolbox we have to use a regression model, instead of a discriminant model. For this we convert the gender factor to a numeric variable before applying the model.
...
The `ropls` package automatically applies cross-validation to asses the performance of the PLSDA model. In `MAPPstructToolbox` this is applied separately to give more control over the approach used if desired. The default cross-validation used by the `ropls` package is 7-fold cross-validation and we replicate that here.
...
## Introduction
The purpose of this vignette is to demonstrate the different functionalities and methods that are available as part of the MAPPstructToolbox and reproduce the data analysis reported in [Mendez et al., (2020)](https://link.springer.com/article/10.1007/s11306-019-1588-0) and [Chan et al., (2016)](https://www.nature.com/articles/bjc2015414).
...
Splitting data into training and test sets is an important aspect of machine learning. In `MAPPstructToolbox` this is implemented using the `split_data` object for random subsampling across the whole dataset, and `stratified_split` for splitting based on group sizes, which is the approach used by Mendez et al.
...
To evaluate the model for discriminant analysis in MAPPstructToolbox the `PLSDA` model is appropriate.
...
Note that the default cutoff in A and B of the figure above for the PLS models in `MAPPstructToolbox` is 0, because groups are encoded as +/-1. This has no impact on the overall performance of the model.

### Permutation test
A permutation test can be used to assess how likely the observed result is to have occurred by chance. In MAPPstructToolbox `permutation_test` is an iterator object that can be combined with other iterators and models.
...
For the `MAPPstructToolbox` package, SVM functionality provided by the `r CRANpkg("e1071")` package has been incorporated into a `model` object. A chart object (`svm_plot_2d`) is also available to plot SVM boundaries for data with two variables.
...
For simplicity we will use a pre-processed version of the 1H-NMR "Gastric cancer" dataset using the `MAPPstructToolbox` package. Details in regards to pre-processing are reported in the "NMR_clinical_metabolomics" vignette of the `r Biocpkg("MAPPstructToolbox") package.
...
## Basic SVM model
The simplest SVM model uses a linear kernel. In `MAPPstructToolbox` the `SVM` model can be used to train and apply SVM models. A `svm_plot_2d` chart object is provided for visualisation of boundaries in two dimensions.
...
## Kernel functions
A number of different kernels can be used with support vector machines. For the `MAPPstructToolbox` wrapper
...
The imported data needs to be converted to `DatasetExperiment` format for use with `MAPPstructToolbox`.
...
### Data transformation
The data is log2 transformed, then scaled such that the mean of the medians is equal for all conditions. These steps are available in `MAPPstructToolbox` using `log_transform` and `mean_of_medians` objects.
...
This specific filter is not in `MAPPstructToolbox` at this time, but can be achieved by combining `filter_na_count` and `filter_by_name` objects. 
